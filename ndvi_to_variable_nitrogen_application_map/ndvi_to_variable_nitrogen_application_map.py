...
class NDVItoVariableNitrogenApplicationMap:
    ...
    def run(self):
        ...
        self.dlg.pushButton_3.clicked.connect(self.convert_N_to_fertilizer)
        
        self.dlg.pushButton_5.clicked.connect(self.export_shapefile)
        
        #self.dlg.lineEdit_13.setEnabled(False)
        #self.dlg.checkBox_8.clicked.connect(self.n_and_fertilizer_amounts)
        ...
        # for now here initial setup colors
        self.dlg.lineEdit_7.setStyleSheet("color: rgb(255, 0, 0);")
        self.dlg.lineEdit_8.setStyleSheet("color: rgb(255, 0, 0);")
        self.dlg.lineEdit_9.setStyleSheet("color: rgb(255, 0, 0);")
        self.dlg.lineEdit_10.setStyleSheet("color: rgb(255, 0, 0);")
        self.dlg.lineEdit_11.setStyleSheet("color: rgb(255, 0, 0);")
        self.dlg.lineEdit_12.setStyleSheet("color: rgb(255, 0, 0);") 
        ...
        # temporaly solution
        self.NtoFertilizserScaliingFlag = False        

        self.dlg.pushButton_4.clicked.connect(self.prepare_combobox_list_for_exprot)
        self.dlg.pushButton_3.clicked.connect(self.prepare_combobox_list_for_exprot)
        self.dlg.pushButton_7.clicked.connect(self.prepare_combobox_list_for_exprot)
    ...
    def guide_to_user(self):
        ...
        + '   - Convert N kg/ha recommendations to fertilizer amounts kg/ha (e.g. CAN fertilizer has 27% N, and based on given % plugin conducts conversion.\n'
    ...
    # def n_and_fertilizer_amounts(self):
        # if self.dlg.checkBox_8.isChecked():
            # self.dlg.lineEdit_13.setEnabled(True)
        # if not self.dlg.checkBox_8.isChecked():
            # self.dlg.lineEdit_13.setEnabled(False)                        
    ...
    def select_variable_Nitrogen_application_flag(self):
        ...
        #disable access to N to fertilizer conversion section
        #self.dlg.checkBox_8.setEnabled(True)
        self.dlg.lineEdit_13.setEnabled(True)
    ...
    def select_variable_Yield_application_flag(self):
        ...
        self.dlg.lineEdit_7.setStyleSheet("color: rgb(0, 0, 255);")
        self.dlg.lineEdit_8.setStyleSheet("color: rgb(0, 0, 255);")
        self.dlg.lineEdit_9.setStyleSheet("color: rgb(0, 0, 255);")
        self.dlg.lineEdit_10.setStyleSheet("color: rgb(0, 0, 255);")
        self.dlg.lineEdit_11.setStyleSheet("color: rgb(0, 0, 255);")
        self.dlg.lineEdit_12.setStyleSheet("color: rgb(0, 0, 255);")        
    ...
    def image_analysis(self):
        ...
        for selectedImage in self.dlg.listWidget_2.selectedItems():
            ...
            for field in polygonLayer.fields():
                ...
                fields.append(idx)
            print ( fields)   
            # this one will remove id indexed at 0 from the list of fields to remove later - while cleaning attribute table
            fields.remove(0)
            print ( fields)
            ...
            if self.dlg.checkBox_6.isChecked():
                ...
                for feature in layer.getFeatures():                    
                    ...
                    try:
                        ...
                        listOfExcludedGrids.append(feature.id())
                ...
                if len(listOfExcludedGrids) != 0:
                    QMessageBox.warning(None, 'The list of grids (features) doesnt overlap with raster image!', 'For this specific layer analyisis: ' + str(selectedLayer1) + ' these features are NOT included: ' + str(listOfExcludedGrids))
            ...
            # if not self.dlg.checkBox_6.isChecked():            
                ...
                # for feature in layer.getFeatures():                    
                    # print ('field', feature.id(), 'value',feature.attributes()[idxIN])
                    # try:
                        # if float(feature.attributes()[idxIN]) <=  myMin and  float(feature.attributes()[idxIN]) >=  myMax:   
                            # layer.startEditing()
                            # layer.deleteFeature(feature.id())
                            # layer.commitChanges()
                    # except:
...
####################
        ...
        # percent difference calcualtion 
        #lowPerc = (float(self.myMeanMeM) - float(self.myMinMeM)) / ((float(self.myMeanMeM) +  float(self.myMinMeM))/2)
        #highPerc = (float(self.myMaxMeM) - float(self.myMeanMeM)) / ((float(self.myMeanMeM) + float(self.myMaxMeM))/2)
...
##################################                
    ...
    def convert_N_to_fertilizer(self):
        print ('convert N to fertilizer kg ha')

        self.dlg.textBrowser.append('\n   Convert N kg/ha to Fertilizer kg/ha - running...')
        ...
        for selectedImage in self.dlg.listWidget_4.selectedItems():
            ...
            for feature in activeLayer.getFeatures():        
                ...
                applicationNrateA = round(float(attrs[idxN]) /(float(self.dlg.lineEdit_13.text())/100), 1)

                print ('value of napp kg fertilizer per grid', applicationNrateA)
        ...
        # temporaly solution
        self.NtoFertilizserScaliingFlag = True
        
        self.dlg.textBrowser.append('   Convert N kg/ha to Fertilizer kg/ha - finished.')
    ...
    def scale_Napp_to_actual_size_of_grid(self):
        ...
        for selectedImage in self.dlg.listWidget_4.selectedItems():
            ...
            Napplication = 'Napp'
            if self.dlg.checkBox_4.isChecked() and self.NtoFertilizserScaliingFlag == True:
                Napplication = 'Fertilizerkg'
                print ('actualFertilizerkg rescalling active')        
            ...
            if self.dlg.checkBox_4.isChecked() and self.NtoFertilizserScaliingFlag == True:
                NapplicationA = 'actualFertilizerkg'
                print ('actualFertilizerkg rescalling active') 
            ...
            for feature in activeLayer.getFeatures():        
                ...
                # if self.dlg.checkBox_8.isChecked():
                    # applicationNrateA = round((float(attrs[idxN]) * ((area.measureArea(geom))/10000))/(float(self.dlg.lineEdit_13.text())/100), 1)
    ...
    def prepare_combobox_list_for_exprot(self):
        print ('comobobox list prepare for exprot')
        ...
        fieldNames = []
        for fieldN in activeLayer.fields():
            if fieldN.name() != 'id':
                fieldNames.append(fieldN.name())
        print ('fieldNames', fieldNames)    
        self.dlg.comboBox_5.addItems(fieldNames)

        
    def export_shapefile(self):
        print ('save application map shape file')

        myfilepathP = str(self.dlg.lineEdit_5.text())
        print ('myfilepathP input image', myfilepathP) 

        try:
            os.mkdir(myfilepathP + '/SaveApplicationMapShapeFile')
        except:
            print ('dir exist')

        myfilepathPd = myfilepathP + '/SaveApplicationMapShapeFile' 
        print ('myfilepathPd', myfilepathPd)
        
        myfilepathPd = os.path.normpath(myfilepathPd)
        print ('myfilepathPd norm', myfilepathPd)

        self.dlg.textBrowser.append('\n   Application map process running...')
        ...
        for selectedImage in self.dlg.listWidget_4.selectedItems():
            ...
            layer.selectAll()
            clone_layer = processing.run("native:saveselectedfeatures", {'INPUT': layer, 'OUTPUT': 'memory:{}'.format(str(selectedLayer)+'_'+str(self.dlg.comboBox_5.currentText()))})['OUTPUT']
            layer.removeSelection()
            QgsProject.instance().addMapLayer(clone_layer)
            
            #selectedLayer = str(self.dlg.listWidget_4.currentItem().text()) + '_' + str(self.dlg.comboBox_5.currentText())            
            activeLayer = QgsProject.instance().mapLayersByName(selectedLayer)[0]
            ...
            keepOnly = ['id', str(self.dlg.comboBox_5.currentText())]
            fields = []
            #fieldnames = set(['id','Napp'])
            ...
            self.dlg.textBrowser.append('\n   Application map saved in - ' + str(myfilepathPd) + ' - directory')
        self.dlg.textBrowser.append('\n   Application map process finished.')
        ...
        # field_name = "Napp"
        # idx = activeLayer.fields().indexOf(field_name)
        # unique_values = activeLayer.uniqueValues(idx)

        # for value in unique_values:
            # file_path = f"{myfilepathPd}/_{value}.shp"
            # activeLayer.selectByExpression(f"{field_name}='{value}'")
            
        # QgsVectorFileWriter.writeAsVectorFormat(activeLayer,
                                                # file_path,
                                                # 'utf-8',
                                                # activeLayer.crs(),
                                                # "ESRI Shapefile",
        ...
        #QgsVectorFileWriter.writeAsVectorFormat(_vlayer,r"C:/Users/myName/xx/hoppla.shp","utf-8",None,"ESRI Shapefile")
        #write = QgsVectorFileWriter.writeAsVectorFormat(activeLayer,r"D:/Users/ememic/Desktop/starzach_25x30_REAL/SaveApplicationMapShapeFile","utf-8",None,"ESRI Shapefile")
        #write = QgsVectorFileWriter.writeAsVectorFormat(activeLayer,str(myfilepathPd) + '//' + str(nameFile), "utf-8",None,"ESRI Shapefile")
        
        #write = QgsVectorFileWriter.writeAsVectorFormat(activeLayer,str(myfilepathPd) + '//' + str(nameFile), "utf-8",driverName="ESRI Shapefile")
        
        #write = QgsVectorFileWriter.writeAsVectorFormat(activeLayer,str(myfilepathPd) + '//' + str(nameFile), "utf-8",driverName="ESRI Shapefile") #, onlySelected=True)